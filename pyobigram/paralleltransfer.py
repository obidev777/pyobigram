...
from .readers import MonitorReader
...
async def download_file(client: TelegramClient,
                        ...
                        progress_args: callable = None
                        ,self_in=None) -> BinaryIO:
    dc_id, location = utils.get_input_location(location)
    # We lock the transfers because telegram has connection count limits
    downloader = ParallelTransferrer(client, dc_id)
    downloaded = downloader.download(location, size)
    monitor = MonitorReader(size)
    filename = str(filepath).split('/')[-1]
    with open(filepath,'wb') as out:
        async for x in downloaded:
            out.write(x)
            if monitor.can_step(len(x)):
                if progress_func:
                    progress_func(self_in,filename,monitor.chunk_por,monitor.total,monitor.size_per_second,monitor.clock_time,progress_args)
                monitor.cleanup()
